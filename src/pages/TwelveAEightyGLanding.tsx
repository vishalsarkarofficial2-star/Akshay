...
import { TwelveAEightyGContentSections, TWELVE_A_FAQ_ITEMS } from './ngo/TwelveAEightyGContentSections';

interface TwelveAEightyGLandingProps {
  onBackToHome: () => void;
...
const TOC_LINKS = [
  ...
  { id: 'what-is-ngo-registration', label: 'What is 12A & 80G' },
...
export const TwelveAEightyGLanding: React.FC<TwelveAEightyGLandingProps> = ({
  onBackToHome,
  onSelectService,
  onOpenBrochure,
  onOpenAppointment
  ...
  const [selectedPackageTier, setSelectedPackageTier] = useState('12A & 80G Registration (₹5,999 + Govt Fee)');
  const [captchaInput, setCaptchaInput] = useState('');
  const [captchaCode, setCaptchaCode] = useState('9V3X5');
  ...
  const handleShare = (platform: string) => {
    ...
    const title = '12A & 80G Registration starting at ₹5999/- only + Govt. Fee! | Akshay B2B Solutions';
    if (platform === 'facebook') {
      ...
      window.open(`mailto:?subject=${encodeURIComponent(title)}&body=${encodeURIComponent('Check out 12A & 80G Registration on Akshay B2B Solutions: ' + url)}`, '_blank');
    }
  };
    ...
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-blue-900 selection:text-white flex flex-col antialiased">
      ...
      <div className="bg-slate-100 border-b border-slate-200 py-2.5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-xs text-slate-600 flex items-center gap-2">
          ...
          <span>»</span>
          <span className="text-blue-900 font-bold">12A &amp; 80G Registration</span>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
            ...
            <div className="lg:col-span-7 space-y-6">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/15 border border-blue-400/30 text-blue-300 text-xs font-semibold">
                ...
                <span>Income Tax Act, 1961 | 100% Tax Exemption &amp; 50% Donor Deduction</span>
              </div>
              ...
              <div className="p-3.5 rounded-xl bg-gradient-to-r from-blue-500/20 via-blue-600/15 to-transparent border-l-4 border-blue-400 text-blue-200 font-bold text-sm sm:text-base">
                Obtain 12A &amp; 80G Registration starting at ₹5999/- only + Govt. Fee!
              </div>

              <p className="text-slate-300 text-sm sm:text-base leading-relaxed">
                12A &amp; 80G Registration are two crucial tax exemption certificates issued by the Income Tax Department under Sections 12A (now 12AB) and 80G of the Income Tax Act, 1961, to eligible non-profit organizations (Trusts, Societies, and Section 8 Companies). Section 12A grants full income tax exemption on the NGO's surplus income/receipts (provided at least 85% is spent on charitable purposes), while Section 80G allows donors to claim a 50% tax deduction on their contributions, significantly boosting the organization's fundraising capacity and credibility with individual and corporate donors.
              </p>
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                {[
                  ...
                  'Form 10A / 10AB',
                  '100% Tax Relief on Surplus',
                  '50% Donor Tax Deduction',
                  'Provisional vs Regular 12AB/80G',
                  'Lifetime Donor Trust'
                ].map((tag, idx) => (
                  <span
                    key={idx}
              ...
              <div className="grid grid-cols-2 gap-3 pt-2 text-xs sm:text-sm text-slate-200">
                <div className="flex items-center gap-2">
                  ...
                  <span>Dual 12A + 80G Filing in One Go</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-blue-400 flex-shrink-0" />
                  <span>Chartered Accountant Supervised Process</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-blue-400 flex-shrink-0" />
                  <span>Form 10A / 10AB Drafting &amp; Submission</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-blue-400 flex-shrink-0" />
                ...
                <div className="border-b border-slate-100 pb-4 mb-5 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                      Fast-track 12A &amp; 80G certification with senior non-profit CA counsel
                    </p>
                  </div>
                ...
                {formSubmitted ? (
                  <div className="py-8 text-center space-y-4">
                    ...
                    <p className="text-xs text-slate-600 max-w-sm mx-auto">
                      Thank you, <strong>{applicantName || 'Valued Non-Profit Leader'}</strong>. Our NGO taxation team will call you shortly on <strong>{applicantMobile}</strong>.
                    </p>
                    <button
                      onClick={() => {
                  ...
                  <form onSubmit={handleFormSubmit} className="space-y-4 text-xs">
                    ...
                    {formStep === 1 && (
                      <div className="space-y-3.5">
                        ...
                        <div>
                          ...
                          <input
                            type="text"
                            placeholder="e.g. CA Sunita Joshi"
                            value={applicantName}
                    ...
                    {formStep === 2 && (
                      <div className="space-y-3.5">
                        <div>
                          <label className="block font-bold text-slate-700 mb-1">
                            Entity Constitution <span className="text-rose-500">*</span>
                          </label>
                          <select
                            value={businessType}
                        ...
                        <div>
                          <label className="block font-bold text-slate-700 mb-1">
                            Primary Sector / Field of Charity <span className="text-rose-500">*</span>
                          </label>
                          <select
                        ...
                        <div className="p-3 bg-blue-50 rounded-xl border border-blue-100 text-slate-700 text-xs">
                          <p className="font-semibold text-blue-900 mb-0.5">Form 10A / 10AB Dual Filing</p>
                          Our CA team formats your charitable objective notes, activity logs, and financial attachments for instant approval.
                        </div>
                      </div>
                    )}
          ...
          <div className="lg:col-span-9 space-y-12">
            ...
            <section id="packages" className="scroll-mt-28 space-y-4 pt-4">
              ...
              <p className="text-xs text-slate-600">
                Guaranteed flat professional fee covering both 12A and 80G tax registrations simultaneously.
              </p>
              ...
              <div className="rounded-3xl border-2 border-blue-600 bg-white p-6 sm:p-8 shadow-xl relative overflow-hidden">
                ...
                <div className="max-w-2xl">
                  ...
                  <p className="text-xs sm:text-sm text-slate-600 mt-1">
                    Dual 12A and 80G tax exemption registrations filed end-to-end with the Income Tax Department, maximizing tax relief and donor fundraising.
                  </p>
                  ...
                  <div className="mt-8 pt-6 border-t border-slate-100">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 text-xs text-slate-700">
                      {[
                        ...
                        'Document Compilation & Audit Review',
                        'Activity Report Preparation',
                        'Provisional / Regular 12A & 80G Certificates',
                        'Timely Service',
                ...
                {[
                  {
                    ...
                    desc: 'Senior Chartered Accountants specializing in NGO taxation drafting your Form 10A/10AB directly.'
                  },
                  {
                    icon: <Clock className="w-5 h-5 text-blue-800" />,
                    title: 'Time-Saving Process',
                    desc: 'Zero paperwork hassles; rapid portal e-filing with swift URN generation.'
                  },
                  {
                    icon: <TrendingUp className="w-5 h-5 text-blue-800" />,
                    title: 'Affordable Pricing',
                    desc: 'Transparent dual-registration pricing with zero hidden surcharges or surprise markups.'
                  },
                  {
                    icon: <Users className="w-5 h-5 text-blue-800" />,
                    title: 'Trusted by Thousands',
                    desc: 'Over 18,500 non-profits, trusts, and foundations supported across India.'
                  },
                  {
                    icon: <Zap className="w-5 h-5 text-blue-800" />,
                    title: 'Compliance Alerts',
                    desc: 'Automated reminders for Form 10BD annual donation filings, ITR-7 deadlines, and 5-year renewals.'
                  },
                  {
                    icon: <Lock className="w-5 h-5 text-blue-800" />,
                    title: 'Secure and Confidential',
                    desc: 'Bank-grade 256-bit encryption for all founder KYC and financial records.'
                  }
                ].map((card, idx) => (
                  <div
                    key={idx}
            ...
            <div className="rounded-3xl bg-gradient-to-r from-blue-950 via-slate-900 to-slate-900 text-white p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-xl">
              <div className="space-y-1 text-center sm:text-left">
                <h3 className="text-lg sm:text-xl font-black">
                  Need 1-on-1 Guidance from a Senior Non-Profit Chartered Accountant?
                </h3>
                <p className="text-xs text-slate-300">
                  Book a confidential 30-minute consultation to review your 12A/80G eligibility, past balance sheets, and Form 10BD compliances.
                </p>
              </div>
              <button
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-base sm:text-lg font-black">
                    Manage Your 12A &amp; 80G Certificates &amp; Donors from Your Phone
                  </h3>
                  <p className="text-xs text-slate-400 mt-0.5">
                    Download the Akshay B2B Android app to monitor Form 10BD filings, generate donor receipts, and consult CA experts on the go.
                  </p>
                </div>
              </div>
              <a
